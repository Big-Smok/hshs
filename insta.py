...
ib = 0
import pyfiglet
P55 = pyfiglet.figlet_format('☆SFAH☆')
...
def pro():
 ...
 '27.79.199.64:16000']
    ...
    # Set environment variables to tunnel all future requests
...
# Now test if proxy is actuall
# Example usage
text = "RAINBOW COLORS"
print(rainbow_text('This tool is by  S F A H | DONE DECODE BY S F A H '))
print("\033[1;94m" + "═" * 72)
blue = "\033[1m\033[34m"
green = "\033[1m\033[32m"
WDEH = render('S F A H', colors=['blue', 'green'], align='center')
print(WDEH)
print('')                                                               
print("\033[1;94m" + "═" * 72)
...
✔️ 𝗩ᴇʀɪғɪᴇᴅ ➟ {is_verified}  
🔒 𝗣ʀɪᴠᴀᴛᴇ ➟ {isPraise}  
══════════════════════════════  
BY : @TIT_F - @TP500 "
══════════════════════════════ 
"""
	with open('ror.txt','a')as ff:ff.write(f"{ss}\n")
	try:
		try:requests.get(f"https://api.telegram.org/bot{token}/sendMessage?chat_id={ID}&text={ss}")
		except:pass
	except Exception as e:pass
	#try:
	#	try:requests.get(f"https://api.telegram.org/bot{owner_token}/sendMessage?chat_id={owner_user_id}&text={ss}")
	#	except:pass
	#except Exception as e:pass
def gg():
	C='user';B='data';A='lsd'
	while _D:
		data={A:''.join(random.choices(string.ascii_letters+string.digits,k=32)),'variables':json.dumps({'id':int(random.randrange(bbk,id)),'render_surface':'PROFILE'}),'doc_id':'25618261841150840'};response=requests.post('https://www.instagram.com/api/graphql',headers={'X-FB-LSD':data[A],"X-Forward-For": "127.0.0.1"},data=data)
		try:
...
for _ in range(200):Thread(target=gg).start()